...
bc_weighted = pd.read_csv('/Volumes/Data2/RST/notebook/BC Results Notebooks/bc_weighted_edge.csv')
bc_weighted = bc_weighted[bc_weighted['type'] == tts_type].sort_values(by = 'weighted', ascending = False
                                                                    ).reset_index().drop(columns = 'index')

bc_weighted = bc_weighted.sample(frac=1).reset_index(drop=True)
...
while removed_edge <= len(bc_weighted):
    ...
    if iteration%10 == 0:
        ...
        with open(per + '/' + num_str + '-ge-log-random.txt', 'a', newline='') as f:
            writer = csv.writer(f, delimiter=',')
            writer.writerow([str(elapsed), str(iteration), str(removed_edge), str(G_copy.ecount())])
            
    if iteration%50 == 0:
        importance_df.to_csv(per + '/' + num_str + '-GE-' + per + '-random.csv', index = False)
